...
#' }
#' @export
radioButtonsColl <- function(inputId, label, hiddenId, hiddenIdBox, hiddenSelected, choices = NULL,
                             hiddenChoices = NULL, selected = NULL,
                             inline = FALSE, width = NULL, choiceNames = NULL, choiceValues = NULL) {
  ...
  tag1 <- tags$div(id = inputId,
           style = if (!is.null(width)) paste0("width: ", validateCssUnit(width), ";"),
           class = divClass,
           controlLabel(inputId, label),
           options
  )
  tag2 <- shinyjs::hidden(div(id=hiddenId,
                      checkboxGroupInput(inputId=hiddenIdBox,
                                         label = NA,
                                         choices = hiddenChoices,
                                         selected = hiddenSelected)))
  return(list("show"=tag1, "hide"=tag2))
}
